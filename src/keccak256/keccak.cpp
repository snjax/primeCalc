...
unsigned char* Keccak::getHash()
{
  ...
  for (unsigned int i = 0; i < hashLength; i++)
    for (unsigned int j = 0; j < 8; j++) // 64 bits => 8 bytes
    {
      ...
      result[k] = (unsigned char) (m_hash[i] >> (8 * j));
	  k++;
    }
  ...
  {
    // convert a byte to hex
    result[k] = (unsigned char) (m_hash[hashLength] >> processed);
    k++;
    processed += 8;
  }
...
unsigned char*  Keccak::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
unsigned char*  Keccak::operator()(const std::string& text)
{
  reset();
